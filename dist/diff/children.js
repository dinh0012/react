...
function diffChildren(parentDom, newParentVNode, oldParentVNode, context, isSvg, excessDomChildren, mounts, ancestorComponent, oldDom) {
  ...
  var newChildren = newParentVNode._children || toChildArray(newParentVNode.props.children, newParentVNode._children = [], _createElement.coerceToVNode, true); // This is a compression of oldParentVNode!=null && oldParentVNode != EMPTY_OBJ && oldParentVNode._children || EMPTY_ARR
  ...
  if (oldDom == _constants.EMPTY_OBJ) {
    oldDom = null;
  ...
  for (i = 0; i < newChildren.length; i++) {
    childVNode = newChildren[i] = (0, _createElement.coerceToVNode)(newChildren[i]);
    ...
    if (childVNode != null) {
      ...
      newDom = (0, _index.diff)(parentDom, childVNode, oldVNode, context, isSvg, excessDomChildren, mounts, ancestorComponent, null, oldDom); // Only proceed if the vnode has not been unmounted by `diff()` above.
  ...
  if (excessDomChildren != null && newParentVNode.type !== _createElement.Fragment) for (i = excessDomChildren.length; i--;) {
    if (excessDomChildren[i] != null) (0, _util.removeNode)(excessDomChildren[i]);
  } // Remove remaining oldChildren if there are any.

  for (i = oldChildrenLength; i--;) {
    if (oldChildren[i] != null) (0, _index.unmount)(oldChildren[i], ancestorComponent);
  }
}
/**
