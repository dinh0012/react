...
function diff(parentDom, newVNode, oldVNode, context, isSvg, excessDomChildren, mounts, ancestorComponent, force, oldDom) {
  ...
  if (tmp = _options["default"].diff) tmp(newVNode);

  try {
    outer: if (oldVNode.type === _createElement.Fragment || newType === _createElement.Fragment) {
      ...
      (0, _children.diffChildren)(parentDom, newVNode, oldVNode, context, isSvg, excessDomChildren, mounts, ancestorComponent, oldDom); // Mark dom as empty in case `_children` is any empty array. If it isn't
      ...
      if (oldVNode._component) {
        ...
        if (newType.prototype && newType.prototype.render) {
          ...
          newVNode._component = c = new _component.Component(newVNode.props, cctx);
      ...
      if (newType.getDerivedStateFromProps != null) {
        (0, _util.assign)(c._nextState == c.state ? c._nextState = (0, _util.assign)({}, c._nextState) : c._nextState, newType.getDerivedStateFromProps(newVNode.props, c._nextState));
      ...
      c.props = newVNode.props;
      c.state = c._nextState;
      if (tmp = _options["default"].render) tmp(newVNode);
      var prev = c._prevVNode || null;
      c._dirty = false;
      var vnode = c._prevVNode = (0, _createElement.coerceToVNode)(c.render(c.props, c.state, c.context));
    ...
    if (tmp = _options["default"].diffed) tmp(newVNode);
  } catch (e) {
...
function commitRoot(mounts, root) {
  ...
  if (_options["default"].commit) _options["default"].commit(root);
}
/**
...
function diffElementNodes(dom, newVNode, oldVNode, context, isSvg, excessDomChildren, mounts, ancestorComponent) {
  ...
  if (newVNode.type === null) {
    ...
    if (excessDomChildren != null && dom.childNodes != null) {
      excessDomChildren = _constants.EMPTY_ARR.slice.call(dom.childNodes);
    }
    ...
    if (newVNode !== oldVNode) {
      ...
      (0, _children.diffChildren)(dom, newVNode, oldVNode, context, newVNode.type === 'foreignObject' ? false : isSvg, excessDomChildren, mounts, ancestorComponent, _constants.EMPTY_OBJ);
      (0, _props.diffProps)(dom, newProps, oldProps, isSvg);
    }
  }
...
function unmount(vnode, ancestorComponent, skipRemove) {
  var r;
  if (_options["default"].unmount) _options["default"].unmount(vnode);
  ...
  if (dom != null) (0, _util.removeNode)(dom);
}
  ...
  for (; component; component = component._ancestorComponent) {
    if (!component._processingException) {
      try {
        ...
        return (0, _component.enqueueRender)(component._pendingError = component);
      } catch (e) {
        error = e;
        isSuspend = false;
      }
